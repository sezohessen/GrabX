...
use Livewire\GenerateSignedUploadUrl as LivewireGenerateSignedUploadUrl;
...
trait WithFileUploads
{
    public function startUpload($name, $fileInfo, $isMultiple)
    {
        if (FileUploadConfiguration::isUsingS3()) {
            ...
            $this->emit('upload:generatedSignedUrlForS3', $name, LivewireGenerateSignedUploadUrl::forS3($file))->self();

            return;
        }
        $genrateURL = new LivewireGenerateSignedUploadUrl;
        $this->emit('upload:generatedSignedUrl', $name, $genrateURL->forLocal())->self();
    }
